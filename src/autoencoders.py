...
class DotaMatchAutoencoder(nn.Module):
    ...
    def __init__(
        ...
        # Cabeças de saída específicas - SEPARADAS
        self.radiant_heroes_output = nn.Linear(self.total_input_dim, self.radiant_heroes_dim)
        self.dire_heroes_output = nn.Linear(self.total_input_dim, self.dire_heroes_dim)
        self.ban_output = nn.Linear(self.total_input_dim, self.ban_features_dim)
        self.stats_output = nn.Linear(self.total_input_dim, self.stats_dim)
    ...
    def decode(self, z: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        return {
            'radiant_heroes': self.radiant_heroes_output(decoded),
            'dire_heroes': self.dire_heroes_output(decoded),
            'bans': self.ban_output(decoded),
            'stats': self.stats_output(decoded)
        }
...
class DotaDataProcessor:
    ...
    def create_model_input(
        ...
        # Embeddings de time
        team_ids = torch.tensor([[0, 1]] * batch_size).view(batch_size, 2)  # [Radiant=0, Dire=1]
...
class DotaAutoencoderTrainer:
    ...
    def train_step(self, x: torch.Tensor) -> float:
        ...
        # Calcular loss de reconstrução para o formato V2 (com separação de times)
        reconstruction_loss = self.criterion(
            torch.cat([decoded['radiant_heroes'], decoded['dire_heroes'], 
                      decoded['bans'], decoded['stats']], dim=1),
            x
        )
    ...
    def evaluate(self, dataloader) -> float:
        ...
        with torch.no_grad():
            for batch in dataloader:
                latent, decoded = self.model(batch)
                loss = self.criterion(
                    torch.cat([decoded['radiant_heroes'], decoded['dire_heroes'],
                              decoded['bans'], decoded['stats']], dim=1),
                    batch
