...
class OptimizedConfig:
    ...
    def _heroes(self) -> pl.LazyFrame:
        ...
        heroes = (
            ...
            .with_columns([
                ...
                *[pl.col(k).cast(v, strict=False).alias(k)
                  for k, v in OptimizedSchema.heroes_schema.items()]
            ])
        )
        ...
        for h in heroes.iter_rows(named=True):
            ...
            rows.append(h)
        return pl.LazyFrame(rows).select(OptimizedSchema.heroes_parsed_schema.keys())
...
class OptimizedDataset:
    def __init__(self, dataset_path: str = '', default_year: int = 2024):
        """
    ...
    def get(self, year: int) -> pl.LazyFrame:
        ...
        self.year = year
        self.check_year()
        cache_dir = f"tmp/cache/dota2_{year}"
        data_parquet_path = os.path.join(cache_dir, "data.parquet")
        if os.path.exists(data_parquet_path):
            log.info(f"Cache encontrado para o ano {year}, carregando do cache...")
            return self.load_cache(cache_dir)
        log.info(f"Cache não encontrado para o ano {year}, processando dados...")
        ...
        metadata.collect().write_parquet(os.path.join(cache_dir, 'metadata.parquet'))
        players.collect().write_parquet(os.path.join(cache_dir, 'players.parquet'))
        ...
        player_aggregate = [pl.col(col).alias(col) for col in OptimizedSchema.players_parsed_schema.keys() if col != "match_id"]
        meta_aggregate = [pl.col(col).first().alias(col) for col in OptimizedSchema.metadata_parsed_schema.keys() if col != "match_id"]
        data = (
            metadata
            .join(players, on="match_id", how="inner")
            .group_by("match_id")
            .agg(player_aggregate + meta_aggregate)
        )
        log.info("Salvando arquivo principal em Parquet...")
        data.collect().write_parquet(data_parquet_path)
    ...
    def load_cache(self, path: str) -> pl.LazyFrame:
        ...
        if not os.path.exists(data_parquet_path):
            log.error(f"Arquivo de cache principal não encontrado em {data_parquet_path}")
            raise FileNotFoundError(f"Arquivo de cache principal não encontrado em {data_parquet_path}")
    ...
    def metadata(self) -> pl.LazyFrame:
        ...
        metadata = (
            ...
            .select(OptimizedSchema.metadata_parsed_schema.keys())
        )
        return metadata
    ...
    def players(self) -> pl.LazyFrame:
        ...
        list_columns = [k for k, v in OptimizedSchema.players_schema.items() if isinstance(v, pl.List)]
        ...
        players = (
            ...
            .with_columns(
                *[pl.col(col).cast(dtype, strict=False).alias(col)
                  for col, dtype in OptimizedSchema.players_schema.items()],
            )
            .with_columns(
                pl.col("hero_id")
        ...
        first_cols = ["match_id", "start_time", "duration"]
        cols = OptimizedSchema.players_parsed_schema.keys()
        log.debug(f"Colunas no schema de jogadores: {cols}")
        log.debug(f"Primeiras colunas: {first_cols}")
        players = (
            self.items_backpack(players)
            .group_by("match_id",)
            .agg([
                pl.col("start_time").first().alias("start_time"),
                pl.col("duration").first().alias("duration"),
                *[pl.col(col) for col in cols if col not in first_cols],
            ])
            .select(OptimizedSchema.players_parsed_schema.keys())
        )
        return players
